...
def translate(
    ...
    with open(template_file, "r") as f:
        try:
            ...
            logging.error(
                "Error parsing TOSCA template: %s %s." % (e.problem, e.context_mark)
            )
            sys.exit(1)
    ...
    with open(map_file, "r") as f:
        try:
            ...
            logging.error(
                "Error parsing NFV mapping: %s %s." % (e.problem, e.context_mark)
            )
            sys.exit(1)

    try:
        tosca_normative_tpl = ToscaNormativeTemplate(
