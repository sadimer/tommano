...
class TestTranslate(unittest.TestCase):
    ...
    def test_validate_small_tpl(self):
        output_tpl, generated_scripts = translate(
            "examples/small_nfv_example.yaml", True, "nfv", "cumulus"
        )
        self.assertEqual(generated_scripts, {})
        self.assertEqual(
            output_tpl,
            {
                "template successfully passed validation": "examples/small_nfv_example.yaml"
            },
        )

    def test_validate_full_tpl(self):
        output_tpl, generated_scripts = translate(
            "examples/full_nfv_example.yaml", True, "nfv", "cumulus"
        )
        self.assertEqual(generated_scripts, {})
        self.assertEqual(
            output_tpl,
            {
                "template successfully passed validation": "examples/full_nfv_example.yaml"
            },
        )

    def test_small_tpl(self):
        output_tpl, generated_scripts = translate(
            "examples/small_nfv_example.yaml", False, "nfv", "cumulus"
        )
        ...
        self.assertEqual(
            output_tpl["tosca_definitions_version"], data["tosca_definitions_version"]
        )
        ...
        for x_key, x in output_tpl["topology_template"]["node_templates"].items():
            ...
            if x["type"] == "tosca.nodes.Compute":
                ...
                self.assertIsInstance(
                    x["interfaces"]["Standard"]["configure"]["inputs"], dict
                )
                self.assertIn(
                    "iPAddressDict", x["interfaces"]["Standard"]["configure"]["inputs"]
                )
                self.assertIsInstance(
                    x["interfaces"]["Standard"]["configure"]["inputs"]["iPAddressDict"],
                    dict,
                )
                self.assertEqual(
                    len(
                        x["interfaces"]["Standard"]["configure"]["inputs"][
                            "iPAddressDict"
                        ]
                    ),
                    2,
                ...
                self.assertIn(
                    "Routing_rules", x["interfaces"]["Standard"]["configure"]["inputs"]
                )
                self.assertIsInstance(
                    x["interfaces"]["Standard"]["configure"]["inputs"]["Routing_rules"],
                    list,
                )
                self.assertEqual(
                    len(
                ...
                self.assertIn(
                    "Routing_routes", x["interfaces"]["Standard"]["configure"]["inputs"]
                )
                self.assertIsInstance(
                    x["interfaces"]["Standard"]["configure"]["inputs"][
                        "Routing_routes"
                ...
                self.assertIn(
                    "implementation", x["interfaces"]["Standard"]["configure"]
                )
                self.assertIn("ports", x["properties"])
                self.assertIsNotNone(x["properties"]["ports"])
                self.assertIsInstance(x["properties"]["ports"], dict)
                self.assertIsNotNone(x["properties"]["ports"]["internal_1"])
                self.assertIsInstance(x["properties"]["ports"]["internal_1"], dict)
                self.assertIsNotNone(
                    x["properties"]["ports"]["internal_1"]["addresses"]
                )
                self.assertIsInstance(
                    x["properties"]["ports"]["internal_1"]["addresses"], list
                )
                self.assertEqual(
                    len(x["properties"]["ports"]["internal_1"]["addresses"]), 2
                )
    ...
    def test_full_tpl(self):
        output_tpl, generated_scripts = translate(
            "examples/full_nfv_example.yaml", False, "nfv", "cumulus"
        )
