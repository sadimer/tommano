...
class TestUtils(unittest.TestCase):
    ...
    def test_tosca_type_parse(self):
        ...
        namespace, category, type_name = utils.tosca_type_parse("tosca.nodes.network.Network")
        self.assertEqual(namespace, "tosca")
        self.assertEqual(category, "nodes")
        self.assertEqual(type_name, "network.Network")

    def test_str_dots_to_dict(self):
        self.assertEqual(utils.str_dots_to_dict("dict1.dict2.dict3", "test"), {"dict1": {"dict2": {"dict3": "test"}}})
        self.assertEqual(utils.str_dots_to_dict("[].dict1.dict2", "test"), [{"dict1": {"dict2": "test"}}])
        self.assertEqual(utils.str_dots_to_dict("dict1.0.dict2", "test"), {"dict1": [{"dict2": "test"}]})
        self.assertEqual(utils.str_dots_to_dict("dict1.dict2.[0]", "test"), {"dict1": {"dict2": ["test"]}})

    def test_str_dots_to_arr(self):
        self.assertEqual(utils.str_dots_to_arr("dict1.dict2.dict3"), ["dict1", "dict2", "dict3"])

    def test_deep_update_dict(self):
        source = {"dict1": {"dict2": {"dict3": "test"}}}
        overrides = {"new_dict1": "test"}
        self.assertEqual(utils.deep_update_dict(source, overrides),
                         {"dict1": {"dict2": {"dict3": "test"}}, "new_dict1": "test"})
        overrides = {"dict1": {"new_dict2": "test"}}
        self.assertEqual(utils.deep_update_dict(source, overrides),
                         {"dict1": {"dict2": {"dict3": "test"}, "new_dict2": "test"}, "new_dict1": "test"})
        overrides = {"dict1": {"dict2": {"new_dict3": "test"}}}
        self.assertEqual(utils.deep_update_dict(source, overrides),
                         {"dict1": {"dict2": {"dict3": "test", "new_dict3": "test"}, "new_dict2": "test"},
                          "new_dict1": "test"})
        source = {"dict1": {"dict2": {"dict3": "test"}}}
